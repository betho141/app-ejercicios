...
def obtener_rutina(nombre_clave):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT id_ejercicio, repeticiones, series, fecha FROM rutinas WHERE nombre_clave = %s ORDER BY fecha DESC LIMIT 10",
        (nombre_clave,))
    datos = cur.fetchall()
    cur.close()
    conn.close()
    return pd.DataFrame(datos, columns=["id_ejercicio", "repeticiones", "series", "fecha"])
    ...
    if nombre_usuario:
        ...
        if not df_rutina.empty:
            ...
            for _, row in df_final.iterrows():
                ...
                st.markdown(f"Fecha asignación: {row['fecha'].strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    if password == "dioses123":
        ...
        if st.button("Guardar rutina personalizada"):
            for nombre_ejercicio in ejercicios_seleccionados:
                ...
                cur.execute(
                    "INSERT INTO rutinas (nombre_clave, id_ejercicio, repeticiones, series, fecha) VALUES (%s, %s, %s, %s, %s)",
                    (usuario_seleccionado, id_ej, repeticiones, series, datetime.now()))
            conn.commit()
